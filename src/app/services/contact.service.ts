...
const STORAGE_KEY: string = 'contacts'
...
export class ContactService {
  ...
  //mock the server
  private _contactsFromStorage = this.storageService.load(STORAGE_KEY)
  private _contacts: ContactModel[] = this._contactsFromStorage || CONTACTS;
  ...
  public loadContacts(filterTxt = null): void {
    let contacts = this._contacts;
    if (filterTxt) {
      contacts = this._filter(contacts, filterTxt)
    }
  ...
  public saveContact(contact: ContactModel) {
    const contactToSave = contact._id ? this._updateContact(contact) : this._addContact(contact)
    this._saveToStorage()
  ...
  private _saveToStorage() {
    this.storageService.save(STORAGE_KEY, this._contacts)
  }
}
